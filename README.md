...
As a User (AAU), I want to sign up for a new account with a username and password, so that I can access my personal profile and manage my exhibitions.
1. AAU, I want to sign in with my existing username and password, so that I can access my account.
2. AAU, I want an easy and consistent way to navigate the site through a navbar, so I can access Sign In, Sign Up, Home, My Exhibitions, or Sign Out depending on whether I'm signed in.
3. AAU, I want to see a homepage with a preview grid of exhibitions when I sign in, so I get an overview of exhibitions right away.
4. AAU, I want to click an "Add New Exhibition" button from the homepage or my exhibitions page, so I can quickly start creating a new exhibition.
5. AAU, I need a dedicated "My Exhibitions" page listing all the exhibitions I've created, so I can easily manage and review what I have created.
6. AAU, I want to see a message like "You have not created any exhibitions yet" when my exhibitions list is empty, so I understand why the page looks empty.
7. AAU, I want to click into a specific exhibition and see its full details — title, location, date, and a list of all its artworks, so I can review the whole collection in one place.
8. AAU, I want to click "Add Artwork" from an exhibition's show page, so I can quickly add artworks to that exhibition without leaving the flow.
9. AAU, I want to view an individual artwork's full details — title, artist, year, medium, price, and image, so I can see everything about that piece.
10. AAU, I want to edit or delete an exhibition I created, so I have full control over managing my data.
11. AAU, I want to edit or delete an individual artwork within my exhibition, so I can correct or remove entries without recreating the whole exhibition.
12. AAU, I want only the exhibitions and artworks I created to show edit/delete controls to me, so that other users can't alter or remove my data.
